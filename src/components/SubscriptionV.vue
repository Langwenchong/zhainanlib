  ...
  computed: {
    content(i) {
      ...
      for (let j = 0; j < 3; j++) {
        if (j === 0) {
          cont.push(
            `从饼状图中不难分析得到您更偏好于"${data[0].name}"和"${data[1].name}"类型的内容,我们在下列推荐中会大幅推荐这类内容,高效减少您寻找的时间成本~`
          );
        } else if (j === 1) {
          cont.push(
            `同时,为了避免审美疲劳,我们也会向您推荐您可能较为不感兴趣的"${data[4].name}"和"${data[5].name}"类型的内容,当然他只会占推荐内容中很小的一部分!`
          );
        } else {
          cont.push(
            `如果您对这个板块的内容感兴趣,不妨续费支持一下我们,现在还有包月打折活动哦!当然,如果有什么更好的建议也可以及时反馈哦,点击右下角的客服按钮为我们建言献策~`
          );
        }
      }
      return cont;
